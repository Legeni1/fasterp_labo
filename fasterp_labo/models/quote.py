...
class CRMQUOTE(models.Model):
    ...
    name = fields.Char()
    medecin_id = fields.Many2one(comodel_name="crm.medecin_partn",
      string="Médécin Partenaire",related='consultation_id.medecin_id',store=True )
    ...
    consultation_id = fields.Many2one(comodel_name="crm.consultation", string="", required=True, )
    montant = fields.Float(string="Montant",related="consultation_id.exam_id.standard_price",store=True )
    paye = fields.Boolean(string=' Payé?',default=False)
    date = fields.Datetime( related='consultation_id.date',store=True )
    paye_id = fields.Many2one(comodel_name="crm.paye_quota", string="", )

    
    @api.model
    def create(self, values):
        values.update({'name': self.env['ir.sequence'].next_by_code('crm.quote')})
        session_self__create = super(CRMQUOTE, self).create(values)
        return session_self__create

class CRMPayeQuota(models.Model):
    _name = 'crm.paye_quota'
    ...
    quota_ids = fields.One2many('crm.quote', 'paye_id', 'Quote-parts',readonly=True,states={'brouillon': [('readonly', False)]},store=True)
    ...
    def validation(self):
        ...
        pdf = self.env.ref('fasterp_labo.actions_quote_parts').render_qweb_pdf(self.ids)
    ...
    def print_fact(self):
        for rec in self:
            return self.env.ref('fasterp_labo.actions_quote_parts').report_action(rec)
    ...
    def _onchange_quotas(self):
        ...
        quotes = self.env['crm.quote'].search(domain)
    ...
    def unlink(self):
        ...
        return super(CRMPayeQuota, self).unlink()

    def mdecins(self):
        med = self.env['crm.medecin_partn'].search([])
        medecins = []
        for m in med:
            medecin = {}
            medecin['id'] = m
